...
class IException extends Exception
{
	...
	{
		$string    = strtr($string,"\\","/");
		$wm_path = strtr(WM::$app->getBasePath(),"\\","/");

		if( strpos($string,$wm_path) !== false)
		{
			$string = str_replace($wm_path,"WM_PATH/",$string);
		}
		return $string;
	}
...
class IError
{
	...
	public static function show($errorData = "",$httpNum = 403)
	{
		...
		{
			$errorObj = new Error(WM::$app,'error');
			call_user_func(array($errorObj,'error'.$httpNum),$errorData);
		}

		//是系统内置的错误机制
		else if(file_exists(WM_PATH.'web/view/'.'error'.$httpNum.$controller->extend))
		{
			$controller->render(WM_PATH.'web/view/'.'error'.$httpNum,$errorData);
		}
