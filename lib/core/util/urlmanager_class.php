...
class IUrl
{
	...
	/**
	 * @brief 将Url从WM支持的一种Url格式转成另一个格式。
	...
	private static function getRouteCache()
	{
		...
		//第一次初始化
		$routeList = isset(WM::$app->config['urlRoute']) ? WM::$app->config['urlRoute'] : array();
		if(empty($routeList))
		{
			self::$urlRoute = false;
			return null;
	...
	public static function creatUrl($url='')
	{
		...
		//获取config里面的url配置
		$rewriteRule = isset(WM::$app->config['rewriteRule'])?WM::$app->config['rewriteRule']:'url';
	...
	public static function getRefRoute()
	{
		...
		{
			$localUrlType = WM::$app->config['rewriteRule'] == 'pathinfo' ? self::UrlPathinfo : self::UrlNative;
	...
	public static function beginUrl($url='')
	{
		...
		$url = $phpurl[1];
		$urlArray = array();
		$rewriteRule = isset(WM::$app->config['rewriteRule'])?WM::$app->config['rewriteRule']:'native';
