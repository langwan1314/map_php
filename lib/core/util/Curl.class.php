...
abstract class Curl {
	...
	/**
	 * get curl instance, support some user define key
	 * @param $url
	...
	private static function getCurlInstance($url, $curl_option=array()){
		...
		if($curl_option[CURLOPT_TIMEOUT] && $max_execution_time > 0 && $curl_option[CURLOPT_TIMEOUT] > $max_execution_time) {
	 ...
	 * @param int $timeout 请求时间
	 * @param array $curl_option
	 * @param null $http_state
	 * @throws \Exception
	 * @return bool|mixed
	 */
	public static function post($url, $data, $timeout = self::DEFAULT_TIMEOUT, $curl_option=array(), &$http_state=null) {
		$opt = array(
			CURLOPT_POST => true,
		...
		$http_state = curl_getinfo($curl, CURLINFO_HTTP_CODE);
