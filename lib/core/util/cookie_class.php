...
{
	//cookie前缀
	private static $pre = 'wm_';

	//默认cookie密钥
	private static $defaultKey = 'G%F';
	...
	public static function set($name, $value='', $time=7, $path='/', $domain=null)
	{
		...
		//setcookie(self::$pre.'safecode', ICrypt::encode(self::cookieId(), self::getKey()), time()+60*60*24*7, $path, $domain);
	...
	{
		$encryptKey  = isset(WM::$app->config['encryptKey']) ? WM::$app->config['encryptKey'] : self::$defaultKey;
