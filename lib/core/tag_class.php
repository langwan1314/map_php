...
class ITag
{
	...
	{
		$this->viewPath = $path;
		return preg_replace_callback('/{(\/?)(\$|fjs|url|admin|www|frontend|yun|webroot|theme|skin|echo|query|widget|foreach|set|include|require|if|elseif|else|while|for|js|code)\s*(:?)([^}]*)}/i', array($this,'translate'), $str);
	}
    /**
     * @brief 处理设定的每一个标签
     * @param array $matches
		...
		if($matches[1]!=='/')
		{
			switch($matches[2].$matches[3])
			{
				...
				case 'include:':
				{
					...
					$tagObj = new ITag();
					$view_file = WM::$app->controller->getViewPath().$fileName;
					$html = file_get_contents($view_file);
					return $tagObj->resolve($html);
				}
				case '$':
                {
                    $str = trim($matches[4]);
				...
				case 'fjs':
				{
					$js_src = '/frontend/'.$matches[4];
					return '<?php echo IJSPackage::getJsHtml('.$js_src.');?>';
				}
                ...
                case 'admin:': return '<?php echo IUrl::creatUrl("")."frontend/app/admin/'.$matches[4].'";?>';
                case 'www:': return '<?php echo IUrl::creatUrl("")."frontend/app/www/'.$matches[4].'";?>';
                case 'frontend:': return '<?php echo IUrl::creatUrl("")."frontend/'.$matches[4].'";?>';
				case 'yun:':
				{
					return '<?php echo YunUpload::YUN_UPLOAD_HOST."'.$matches[4].'";?>';
				}
