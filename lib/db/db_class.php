	...
	*/
    ...
    public function query($sql)
    {
        ...
        if(self::$sqlType=='select' || self::$sqlType=='show')
        {
            ...
            {
				//多数据库支持并且读写分离
                if(isset(WM::$app->config['DB']['read']))
                {
					//获取ip地址
					$ip = IClient::getIP();
                    self::$rTarget = $this->connect(IHash::hash(WM::$app->config['DB']['read'],$ip));
                }
                else
                {
                	self::$rTarget = $this->connect(WM::$app->config['DB']);
                }
            }
            $this->switchLink("r");
        ...
        else
        {
            ...
            {
				//多数据库支持并且读写分离
                if(isset(WM::$app->config['DB']['write']))
                {
                	self::$wTarget = $this->connect(WM::$app->config['DB']['write']);
                }
                else
                {
                	self::$wTarget = $this->connect(WM::$app->config['DB']);
                }
