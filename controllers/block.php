...
class Block extends IController
{
    ...
    public static function show_spec($specJson)
    {
    	...
    	foreach($specArray as $val)
    	{
    		...
    		{
    			$spec[$val['name']] = '<img src="'.$bc->getImg($val['value'], 15, 15).'" class="img_border" style="width:15px;height:15px;" />';
    		}
    	}
    	return $spec;
    }
    ...
    */
	public function doPay()
	{
		...
		if ($orderRow['pay_type'] == 13) {//微信扫描支付
			$this->redirect("/pay/weixin?order_no={$orderRow['order_no']}", true, array('order_no'=>$orderRow['order_no']));
		}
		//获取支付方式类库
