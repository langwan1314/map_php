...
class Simple extends IController
{
    ...
    function login_act()
    {
    	...
    	$password   = md5(IReq::get('password','post'));
    	...
    	else
    	{
    		if($userRow = CheckRights::isValidUser($login_info,$password))
    		{
				...
				//更新购物车
				$cart = new Cart();
				$cart->updateCartWhenUserLogin($userRow['user_id']);
    ...
    function joinCart()
    {
		...
		$select    = IFilter::act(IReq::get('select'),'int');

		//加入购物车
    	$cartObj   = new Cart();
    	$addResult = $cartObj->add($goods_id, $goods_num, $type, $select);
    ...
    function promotionRuleAjax()
    {
		...
		$cartObj->setMyCart($cartInfo);
		$countSumObj    = new CountSum();

		$buyInfo = $cartObj->cartFormat($cartObj->filterCartUnSelectedGoods($cartInfo));

		$promptMessage = '获取成功';
		$countSumResult = $countSumObj->goodsCount($buyInfo, '', '', $promptMessage);

		$result = array(
			'isError' => !!$countSumResult,
    ...
    function cart2()
    {
		...
		$result = $countSumObj->cart_count($id,$type,$buy_num,$promo,$active_id, true);
    ...
    function cart3()
    {
		...
		//计算费用
    	$countSumObj = new CountSum($user_id);
		$goodsResult = $countSumObj->cart_count($gid,$type,$num,$promo,$active_id, true);
	...
	function find_password_mobile()
	{
		...
		$userRow = $userDB->getObj('u.username = "'.$username.'" and u.mobile = "'.$mobile.'"');
		if($userRow)
		{
