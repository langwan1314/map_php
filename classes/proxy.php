...
{
	//升级URL
	const UPDATE_URL = 'http://product.crycoder.com/index.php?';
	...
	{
		return include(WM::$app->getBasePath().'docs/version.php');
	}
	...
	{
		$wmshopAuthorize = ISafe::get('wmshopAuthorize');
		if($wmshopAuthorize === null)
		{
			$return = self::send('_c=system&_a=authorize&host='.IUrl::getHost());
			$wmshopAuthorize = isset($return['success']) && $return['success'] == 1 ? true : false;
			ISafe::set('wmshopAuthorize',$wmshopAuthorize);
		}
		return $wmshopAuthorize;
	}
}