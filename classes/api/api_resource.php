...
return array(
	...
	'getCommendHot' => array(
		'query' => array(
			...
			'fields' => 'go.img,go.store_nums,go.sell_price,go.name,go.id,go.market_price,go.sale',
			'limit'=>'10',
			'order'=>'sort asc,id desc'
		)
	),
	//热卖商品列表
	'getCommendHotByCategoryId' => array(
		'query' => array(
			'name' => 'commend_goods as co',
			'join' => 'left join goods as go on co.goods_id = go.id left join category_extend as ca on co.goods_id=ca.goods_id',
			'where' => 'co.commend_id = 3 and go.is_del = 0 AND go.id is not null and ca.category_id in (#category_id#)',
			'fields' => 'go.img,go.sell_price,go.name,go.id,go.market_price,ca.category_id',
			'limit'=>'10',
			'order'=>'sort asc,id desc'
		)
	),
