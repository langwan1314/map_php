...
class CountSum
{
	...
	public function __construct($user_id = 0)
	{
		...
		{
			$userCheckRights = WM::$app->getController()->user;
	 ...
	 * @param string $promo string 活动类型 团购，抢购
	 * @param string $active_id
	 * @param string $errorMessage 出错信息
	 * @return array or bool
	 * @active_id int 活动ID
	 */
	public function goodsCount($buyInfo, $promo='', $active_id='', &$errorMessage='')
	{
		$this->sum           = 0;       //原始总额(优惠前)
    	...
    	else
    	{
	    	...
	    	if(isset($buyInfo['goods']['id']) && $buyInfo['goods']['id'])
	    	{
	    		...
	    		foreach($goodsList as $key => $val)
	    		{
	    			...
	    			{
						$errorMessage =  "商品：".$val['name']."购买数量超出库存，请重新调整购买数量";
						return false;
	    			}
	    			...
	    			$goodsList[$key]['select']  = isset($buyInfo['goods']['data'][$val['goods_id']]['select']) ? $buyInfo['goods']['data'][$val['goods_id']]['select'] : 1;
	    	...
	    	if(isset($buyInfo['product']['id']) && $buyInfo['product']['id'])
	    	{
	    		...
	    		foreach($productList as $key => $val)
	    		{
	    			...
	    			{
						$errorMessage =  "货品：".$val['name']."购买数量超出库存，请重新调整购买数量";
						return false;
	    			}
	    			...
	    			$productList[$key]['select']  = isset($buyInfo['product']['data'][$val['product_id']]['select']) ? $buyInfo['product']['data'][$val['product_id']]['select'] : 1 ;
	...
	//购物车计算
	public function cart_count($id = '',$type = '',$buy_num = 1,$promo='',$active_id='', $isIgnoreUnselectedGoods=false)
	{
		//单品购买
		if($id && $type)
		{
		...
		else
		{
			//获取购物车中的商品和货品信息
	    	$cartObj = new Cart();
	    	$buyInfo = $cartObj->getMyCart($isIgnoreUnselectedGoods);
		}
	 ...
	 * @param float|int $discount float 订单的加价或者减价
	 * @param $promo string 促销活动
	 * @param int|string $active_id int 促销活动ID
	 * @return array $result 最终的返回数组
	 * @throws IException
	 */
    public function countOrderFee($goodsResult,$province_id,$delivery_id,$payment_id,$is_invoice,$discount = 0,$promo = '',$active_id = '')
    {
    	...
    	foreach($sellerGoods as $seller_id => $item)
    	{
    		...
    		foreach($item as $key => $val)
    		{
	    		...
	    		if($val['product_id'] > 0)
	    		{
				    $productArray[$val['product_id']] = array(
					    'num' => $val['count'],
					    'select' => $val['select']
				    );
	    		}
    ...
    public static function countSellerOrderFee($orderList)
    {
		...
		//获取结算手续费
		$result['commissionPer'] = isset(WM::$app->config['commission']) ? WM::$app->config['commission'] : 0;
