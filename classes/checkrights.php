...
class CheckRights extends IInterceptorBase
{
	...
	{
		$controllerInstance = WM::$app->getController();
	...
	/**
	 * @brief  校验注册用户身份信息
	 * @param  string $mobile|用户名|邮箱     用户注册手机号
	 * @param  string $password   用户名的md5密码
	 * @return array or false 如果合法则返回用户数据;不合法返回false
	 */
	public static function isValidUser($mobile,$password)
	{
		$mobile = IFilter::act($mobile);
		$password = IFilter::act($password);

		$userObj = new IModel('user as u,member as m');
		$where   = "(u.username = '{$mobile}' or u.mobile = '{$mobile}' or u.email = '{$mobile}') and m.status = 1 and u.id = m.user_id";
    ...
    public static function loginAfter($userRow)
    {
	    ...
	    $user_name = $userRow['username'] ?: $userRow['mobile'];
	    ISafe::set('username',$user_name);
	...
	/**
	 * @brief app登陆后动作: 生成用户鉴权skey+更新数据
	 * @param array $userRow 用户数组信息
	 */
    public static function generateSey($userRow)
    {
        $hashids = new Hashids\Hashids(WM::$app->config['encryptKey'], 32);
        return $hashids->encode(time() + intval($userRow['id']));
    }
	 ...
	 * @param array $userRow 用户数组信息
	 */
    public static function appLoginAfter($userRow)
    {
        $skey = self::generateSey($userRow);

        $last_login = 0;
        if (isset($userRow['last_login']) && $userRow['last_login'] != '0000-00-00 00:00:00')
        {
            $last_login = strtotime($userRow['last_login']);
        }
		...
		//用户私密数据
		ISafe::set('skey_'.$userRow['id'], $user, 'redis');

		//更新最后一次登录时间
		$memberObj = new IModel('member');
		$dataArray = array(
			'last_login' => strtotime(ITime::getDateTime())
		);
		$memberObj->setData($dataArray);
		$where     = 'user_id = '.$userRow["id"];
		$memberObj->update($where);
    ...
    public static function appLogoutAfter($uid = null)
    {
        if (!$uid)
        {
            $uid = IFilter::act(IReq::get('uid'), 'int');
        }
        //$skey = IFilter::act(IReq::get('skey'));
        ISafe::clear('skey_'.$uid, 'redis');
    }

	/**
	 * [接口]获取APP登录用户信息
	 */
	public static function checkAppUserRights()
	{
		$object = WM::$app->getController();
		$object->user = null;
        ...
        if (!$uid || !$skey || strlen($skey) != 32)
        {
            return;
        }

        $user = ISafe::get('skey_'.$uid, 'redis');
        if ($user != false && $user['skey'] == $skey)
        {
            $user['user_id'] = $uid;
            $object->user = $user;
        }
