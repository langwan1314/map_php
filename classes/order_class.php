...
class Order_Class
{
	 ...
	 * @param int|string $admin_id int    管理员ID
	 * @param $note     string 收款的备注
	 * @param string $pay_id
	 * @return false or int order_id
	 */
	public static function updateOrderStatus($orderNo,$admin_id = '',$note = '', $pay_id='')
	{
		//获取订单信息
    ...
    public static function ifUserCanDel($order)
    {
        if (! $order)
        {
            return false;
        ...
        // 3取消订单(客户触发),4作废订单(管理员触发),5完成订单,6退款,7部分退款
        return in_array($order['status'], [3, 4, 5, 6, 7]);
    }
}
