<?php
/**
 * @brief 商品采集器与Crycoder 外观模式
 * @author windy
 * @date 2015/1/2 8:25:15
 */
class collect_facade
{
	...
	public static function createInstance()
	{
		...
		{
			$pluginFile = WM::$app->getBasePath().'plugins/collect/collect.php';
			if(is_file($pluginFile))
			{
				include_once($pluginFile);
				self::$instance = new jd_collect();
			}
			else
			{
				self::$error = '当前版本没有此功能 <a href="http://www.crycoder.com/buy" target="_blank">立即购买</a>';
			}
		}
			...
			if(isset($val['img']) && $val['img'])
			{
				foreach($val['img'] as $img)
				{
					...
					if(!$photoData)
					{
						...
						$copyResult = IFile::copy($img,WM::$app->getBasePath()."/".$destFile);
						if($copyResult)
						{
