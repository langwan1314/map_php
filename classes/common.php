...
class Common
{
	...
	public static function isThemeUsed($plan)
	{
		if(isset(WM::$app->config['theme']))
		{
			...
			{
				if(in_array($plan,WM::$app->config['theme']))
				{
					return true;
				}
			}
			elseif(WM::$app->config['theme'] == $plan)
			{
				return true;
			}
		}
	...
	public static function isSkinUsed($theme,$plan)
	{
		...
		foreach(WM::$app->config['theme'] as $key => $val)
		{
			if($theme == $val)
			{
				$client = $key;
		...
		if($client)
		{
			$selectPlan = WM::$app->config['skin'][$client];
		}
	...
	{
		//兼容老式主题，仅有PC平台的非数组形式
		$theme1 = is_array(WM::$app->config['theme']) ? WM::$app->config['theme'] : array('pc' => WM::$app->config['theme']);
	...
	{
		//兼容老式主题，仅有PC平台的非数组形式
		$skinArray = is_array(WM::$app->config['skin']) ? WM::$app->config['skin'] : array('pc' => WM::$app->config['skin']);

		//根据其theme查询出所属于的平台
		foreach(WM::$app->config['theme'] as $client => $themeParam)
		{
			if($themeParam == $theme)
			{
				$skinArray = array_merge($skinArray,array($client => $skin));
