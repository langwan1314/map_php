...
class DBBackup
{
	...
	private $fPrefix      = 'crycoder';        //备份文件名前缀
	private $fExtend      = '.sql';            //备份文件扩展名
	private $tablePre     = 'crycoder_';       //表前缀
	...
	function __construct($ctrlRes = null)
	{
		...
		if(isset(WM::$app->config['dbbackup']) && WM::$app->config['dbbackup']!=null)
		{
			$this->dir = WM::$app->config['dbbackup'];
		}

		if(isset(WM::$app->config['DB']['tablePre']))
		{
			$this->tablePre = WM::$app->config['DB']['tablePre'];
		}
